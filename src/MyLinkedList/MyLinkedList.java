...
public class MyLinkedList implements List<Integer>, Deque<Integer> {
    ...
    // определение индекса существующего элемента (иначе: - 1)
    @Override
    ...
    // замена элемента под определенным индексом
    @Override
    ...
    // удаляет элемент под определенным индексом
    @Override
    ...
    public int lastIndexOf(Object o) {
        int index = size;
        if (o == null) {
            for (Node<Integer> x = last; x != null; x = x.prev) {
                index--;
                if (x.item == null)
                    return index;
            }
